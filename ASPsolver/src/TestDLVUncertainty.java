...
public class TestDLVUncertainty {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String f = fixPath(new File("resources/dl/FASE15/SM2HSM_FASE_6U.dl").getPath());
		...
		while (myEngine.hasMoreModel()) {
			...
			while(it.hasNext()) {
				...
				if(current.startsWith("uncertainty_point_edge") && current.contains(metamodelName)) {
					props += current.replaceAll("uncertainty_point_edge", "u-point-edge") + ".\n";
				}
				if(!isSourceSet) {
			...
			result += nodes + edges + uedges + props;
		}
