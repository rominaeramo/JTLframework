 ...
 */
public class JTLMMOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "JTLMM.resource.JTLMM.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "JTLMM.resource.JTLMM.ui.occurences.declaration";
	...
	private JTLMM.resource.JTLMM.ui.JTLMMTokenScanner tokenScanner;
	...
	private JTLMM.resource.JTLMM.IJTLMMTextResource textResource;
	 ...
	 */
	public JTLMMOccurrence(JTLMM.resource.JTLMM.IJTLMMTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, JTLMM.resource.JTLMM.ui.JTLMMTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new JTLMM.resource.JTLMM.mopp.JTLMMMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(JTLMM.resource.JTLMM.ui.JTLMMBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, JTLMM.resource.JTLMM.ui.JTLMMPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, JTLMM.resource.JTLMM.ui.JTLMMPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, JTLMM.resource.JTLMM.ui.JTLMMPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == JTLMM.resource.JTLMM.ui.JTLMMPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(JTLMM.resource.JTLMM.ui.JTLMMOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(JTLMM.resource.JTLMM.ui.JTLMMOccurrence.DECLARATION_ANNOTATION_ID);
	}
