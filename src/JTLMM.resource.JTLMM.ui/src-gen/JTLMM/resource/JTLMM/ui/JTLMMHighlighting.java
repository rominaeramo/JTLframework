 ...
 */
public class JTLMMHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	
	private final static JTLMM.resource.JTLMM.ui.JTLMMPositionHelper positionHelper = new JTLMM.resource.JTLMM.ui.JTLMMPositionHelper();
	...
	private JTLMM.resource.JTLMM.ui.JTLMMTokenScanner scanner;
	...
	private JTLMM.resource.JTLMM.ui.JTLMMOccurrence occurrence;
	private JTLMM.resource.JTLMM.ui.JTLMMBracketSet bracketSet;
	 ...
	 */
	private final class UpdateHighlightingListener implements org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.MouseListener, JTLMM.resource.JTLMM.IJTLMMBackgroundParsingListener {
	 ...
	 * @param editor
	 */
	public JTLMMHighlighting(JTLMM.resource.JTLMM.IJTLMMTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, JTLMM.resource.JTLMM.ui.JTLMMColorManager colorManager, JTLMM.resource.JTLMM.ui.JTLMMEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = JTLMM.resource.JTLMM.ui.JTLMMUIPlugin.getDefault().getPreferenceStore();
		this.editor = editor;
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new JTLMM.resource.JTLMM.ui.JTLMMTokenScanner(textResource, colorManager);
		occurrence = new JTLMM.resource.JTLMM.ui.JTLMMOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new JTLMM.resource.JTLMM.ui.JTLMMBracketSet(editor, sourceviewer);
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(JTLMM.resource.JTLMM.ui.JTLMMPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, JTLMM.resource.JTLMM.ui.JTLMMPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		black = colorManager.getColor(new org.eclipse.swt.graphics.RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(JTLMM.resource.JTLMM.ui.JTLMMEditor editor) {
	...
	private void setBracketHighlighting(org.eclipse.jface.text.IDocument document) {
		...
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, JTLMM.resource.JTLMM.ui.JTLMMPositionCategory.BRACKET.toString());
	...
	private void removeHighlighting() {
		...
		removeHighlightingCategory(document, JTLMM.resource.JTLMM.ui.JTLMMPositionCategory.BRACKET.toString());
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(JTLMM.resource.JTLMM.ui.JTLMMPositionCategory.BRACKET.toString())) {
		...
		display.asyncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new JTLMM.resource.JTLMM.ui.JTLMMEObjectSelection(selectedEObject, false));
				}
			}
		});
	}
