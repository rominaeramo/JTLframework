 ...
 * code).
 */
public class JTLMMDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private JTLMM.resource.JTLMM.debug.AbstractJTLMMDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, JTLMM.resource.JTLMM.util.JTLMMPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, JTLMM.resource.JTLMM.util.JTLMMPair<String, Object>>();
	...
	public JTLMMDebuggerListener(int requestPort) {
		super();
	...
	private void runDebugger() throws java.io.IOException {
		...
		JTLMM.resource.JTLMM.debug.JTLMMDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(JTLMM.resource.JTLMM.debug.EJTLMMDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(JTLMM.resource.JTLMM.debug.EJTLMMDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(JTLMM.resource.JTLMM.debug.EJTLMMDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(JTLMM.resource.JTLMM.debug.EJTLMMDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(JTLMM.resource.JTLMM.debug.EJTLMMDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(JTLMM.resource.JTLMM.debug.EJTLMMDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(JTLMM.resource.JTLMM.debug.EJTLMMDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(JTLMM.resource.JTLMM.debug.EJTLMMDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = JTLMM.resource.JTLMM.util.JTLMMStringUtil.encode('#', stack);
				JTLMM.resource.JTLMM.debug.JTLMMDebugMessage message = new JTLMM.resource.JTLMM.debug.JTLMMDebugMessage(JTLMM.resource.JTLMM.debug.EJTLMMDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(JTLMM.resource.JTLMM.debug.EJTLMMDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				JTLMM.resource.JTLMM.debug.JTLMMDebugMessage message = new JTLMM.resource.JTLMM.debug.JTLMMDebugMessage(JTLMM.resource.JTLMM.debug.EJTLMMDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(JTLMM.resource.JTLMM.debug.EJTLMMDebugMessageTypes.GET_VARIABLES)) {
				...
				JTLMM.resource.JTLMM.debug.JTLMMDebugMessage message = new JTLMM.resource.JTLMM.debug.JTLMMDebugMessage(JTLMM.resource.JTLMM.debug.EJTLMMDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return JTLMM.resource.JTLMM.util.JTLMMStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		JTLMM.resource.JTLMM.util.JTLMMPair<String, Object> pair = new JTLMM.resource.JTLMM.util.JTLMMPair<String, Object>(name, value);
	...
	public JTLMM.resource.JTLMM.debug.AbstractJTLMMDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(JTLMM.resource.JTLMM.debug.AbstractJTLMMDebuggable debuggable) {
