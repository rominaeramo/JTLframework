 ...
 */
public class JTLMMDebuggableInterpreter<ResultType, ContextType> extends JTLMM.resource.JTLMM.debug.AbstractJTLMMDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private JTLMM.resource.JTLMM.util.AbstractJTLMMInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private JTLMM.resource.JTLMM.IJTLMMCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public JTLMMDebuggableInterpreter(JTLMM.resource.JTLMM.util.AbstractJTLMMInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new JTLMM.resource.JTLMM.IJTLMMInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = JTLMM.resource.JTLMM.util.JTLMMStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public JTLMM.resource.JTLMM.util.AbstractJTLMMInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		JTLMM.resource.JTLMM.IJTLMMLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private JTLMM.resource.JTLMM.IJTLMMLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		JTLMM.resource.JTLMM.IJTLMMCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(JTLMM.resource.JTLMM.debug.EJTLMMDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = JTLMM.resource.JTLMM.util.JTLMMEObjectUtil.getDepth(current);
		stopCondition = new JTLMM.resource.JTLMM.IJTLMMCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = JTLMM.resource.JTLMM.util.JTLMMEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = JTLMM.resource.JTLMM.util.JTLMMEObjectUtil.getDepth(current) - 1;
		stopCondition = new JTLMM.resource.JTLMM.IJTLMMCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
