...
public class JTLMMDebugTarget extends JTLMM.resource.JTLMM.debug.JTLMMDebugElement implements org.eclipse.debug.core.model.IDebugTarget, JTLMM.resource.JTLMM.debug.IJTLMMDebugEventListener {
	
	/**
	 * associated system process
	 */
	private JTLMM.resource.JTLMM.debug.JTLMMDebugProcess process;
	...
	private JTLMM.resource.JTLMM.debug.JTLMMDebugThread thread;
	 ...
	 * event listeners
	 */
	private java.util.List<JTLMM.resource.JTLMM.debug.IJTLMMDebugEventListener> eventListeners = new java.util.ArrayList<JTLMM.resource.JTLMM.debug.IJTLMMDebugEventListener>();
	
	private JTLMM.resource.JTLMM.debug.JTLMMDebugProxy debugProxy;
	...
	private class DebugEventDispatcher extends org.eclipse.core.runtime.jobs.Job {
		...
		public DebugEventDispatcher() {
			super(new JTLMM.resource.JTLMM.mopp.JTLMMMetaInformation().getSyntaxName() + " Event Dispatch");
			setSystem(true);
		}
		
		protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
			while (!isTerminated()) {
				JTLMM.resource.JTLMM.debug.JTLMMDebugMessage message = communicationHelper.receive(eventReader);
				if (message != null) {
		...
		private void notifyListeners(JTLMM.resource.JTLMM.debug.JTLMMDebugMessage message) {
			Object[] listeners = eventListeners.toArray();
			for (int i = 0; i < listeners.length; i++) {
				((JTLMM.resource.JTLMM.debug.IJTLMMDebugEventListener) listeners[i]).handleMessage(message);
			}
		}
	}
	
	public JTLMMDebugTarget(JTLMM.resource.JTLMM.debug.JTLMMDebugProcess process, org.eclipse.debug.core.ILaunch launch, int requestPort, int eventPort) {
		super(launch.getDebugTarget());
		this.process = process;
		this.launch = launch;
		this.thread = new JTLMM.resource.JTLMM.debug.JTLMMDebugThread(this);
		this.threads = new org.eclipse.debug.core.model.IThread[] {this.thread};
		
		// initialize debug proxy
		try {
			this.debugProxy = new JTLMM.resource.JTLMM.debug.JTLMMDebugProxy(this, requestPort);
	 ...
	 */
	public void addEventListener(JTLMM.resource.JTLMM.debug.IJTLMMDebugEventListener listener) {
	 ...
	 * @param listener event listener
	 */
	public void removeEventListener(JTLMM.resource.JTLMM.debug.IJTLMMDebugEventListener listener) {
		eventListeners.remove(listener);
	}
	
	public String getName() throws org.eclipse.debug.core.DebugException {
		return new JTLMM.resource.JTLMM.mopp.JTLMMMetaInformation().getSyntaxName() + " model";
	}
		...
		if (supportsBreakpoint(breakpoint)) {
			try {
				if ((breakpoint.isEnabled() && getBreakpointManager().isEnabled()) || !breakpoint.isRegistered()) {
					JTLMM.resource.JTLMM.debug.JTLMMLineBreakpoint lineBreakpoint = (JTLMM.resource.JTLMM.debug.JTLMMLineBreakpoint) breakpoint;
	...
	public JTLMM.resource.JTLMM.debug.JTLMMDebugProxy getDebugProxy() {
