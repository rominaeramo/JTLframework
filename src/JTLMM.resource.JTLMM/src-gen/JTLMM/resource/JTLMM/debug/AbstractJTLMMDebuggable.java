...
public abstract class AbstractJTLMMDebuggable {
	
	/**
	 * The list of breakpoints, where each breakpoint is represented by its location
	 * (a string) and the line number
	 */
	private java.util.List<JTLMM.resource.JTLMM.util.JTLMMPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<JTLMM.resource.JTLMM.util.JTLMMPair<String, Integer>>();
	...
	public void startEventSocket(int eventPort) {
		try {
			...
			new JTLMM.resource.JTLMM.util.JTLMMRuntimeUtil().logError("Can't create socket connection while launching.", e);
		}
	}
	...
	public void stopEventSocket() {
		try {
			...
			new JTLMM.resource.JTLMM.util.JTLMMRuntimeUtil().logError("Exception while closing socket.", e);
		}
	}
	
	public void sendEvent(JTLMM.resource.JTLMM.debug.EJTLMMDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			JTLMM.resource.JTLMM.debug.JTLMMDebugMessage message = new JTLMM.resource.JTLMM.debug.JTLMMDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				JTLMM.resource.JTLMM.util.JTLMMPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(JTLMM.resource.JTLMM.debug.EJTLMMDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(JTLMM.resource.JTLMM.debug.EJTLMMDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new JTLMM.resource.JTLMM.util.JTLMMPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new JTLMM.resource.JTLMM.util.JTLMMPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(JTLMM.resource.JTLMM.debug.EJTLMMDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(JTLMM.resource.JTLMM.debug.EJTLMMDebugMessageTypes.STARTED, true);
			setSuspend(true);
