...
public class JTLMMResourceFactoryDelegator implements org.eclipse.emf.ecore.resource.Resource.Factory {
	
	protected java.util.Map<String, org.eclipse.emf.ecore.resource.Resource.Factory> factories = null;
	
	public JTLMMResourceFactoryDelegator() {
		init();
	}
	...
	protected void init() {
		...
		if (new JTLMM.resource.JTLMM.util.JTLMMRuntimeUtil().isEclipsePlatformAvailable()) {
			new JTLMM.resource.JTLMM.util.JTLMMEclipseProxy().getResourceFactoryExtensions(factories);
		}
		if (factories.get("") == null) {
			factories.put("", new JTLMM.resource.JTLMM.mopp.JTLMMResourceFactory());
		}
	}
