...
/**
 * The JTLMMTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * JTLMMTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class JTLMMTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<JTLMM.resource.JTLMM.mopp.JTLMMTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<JTLMM.resource.JTLMM.mopp.JTLMMTaskItem> foundItems = new java.util.ArrayList<JTLMM.resource.JTLMM.mopp.JTLMMTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new JTLMM.resource.JTLMM.mopp.JTLMMTaskItem(keyword, message, localLine, offset, end));
