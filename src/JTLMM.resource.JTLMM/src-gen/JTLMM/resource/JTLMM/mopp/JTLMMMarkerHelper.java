 ...
 * contains <code>JTLMM.resource.JTLMM.IJTLMMTextDiagnostic</code>s it uses the
 * more precise information of this extended diagnostic type.
 */
public class JTLMMMarkerHelper {
	
	/**
	 * The extension id of the custom marker type that is used by this text resource.
	 */
	public static final String MARKER_TYPE = JTLMM.resource.JTLMM.mopp.JTLMMPlugin.PLUGIN_ID + ".problem";
	...
	private static class MarkerCommandQueue {
		...
		private java.util.List<JTLMM.resource.JTLMM.IJTLMMCommand<Object>> commands = new java.util.ArrayList<JTLMM.resource.JTLMM.IJTLMMCommand<Object>>();
		
		private MutexRule schedulingRule = new MutexRule();
		
		public void addCommand(JTLMM.resource.JTLMM.IJTLMMCommand<Object> command) {
		...
		public void runCommands() {
			java.util.List<JTLMM.resource.JTLMM.IJTLMMCommand<Object>> commandsToProcess = new java.util.ArrayList<JTLMM.resource.JTLMM.IJTLMMCommand<Object>>();
			synchronized(commands) {
				commandsToProcess.addAll(commands);
				commands.clear();
			}
			for (JTLMM.resource.JTLMM.IJTLMMCommand<Object> command : commandsToProcess) {
	 ...
	 */
	public void mark(org.eclipse.emf.ecore.resource.Resource resource, JTLMM.resource.JTLMM.IJTLMMTextDiagnostic diagnostic) {
	...
	protected void createMarkerFromDiagnostic(final org.eclipse.core.resources.IFile file, final JTLMM.resource.JTLMM.IJTLMMTextDiagnostic diagnostic) {
		final JTLMM.resource.JTLMM.IJTLMMProblem problem = diagnostic.getProblem();
		JTLMM.resource.JTLMM.JTLMMEProblemType problemType = problem.getType();
		final String markerID = getMarkerID(problemType);
		COMMAND_QUEUE.addCommand(new JTLMM.resource.JTLMM.IJTLMMCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					if (problem.getSeverity() == JTLMM.resource.JTLMM.JTLMMEProblemSeverity.ERROR) {
					...
					JTLMM.resource.JTLMM.IJTLMMTextDiagnostic textDiagnostic = (JTLMM.resource.JTLMM.IJTLMMTextDiagnostic) diagnostic;
					...
					if (diagnostic instanceof JTLMM.resource.JTLMM.mopp.JTLMMResource.ElementBasedTextDiagnostic) {
						org.eclipse.emf.ecore.EObject element = ((JTLMM.resource.JTLMM.mopp.JTLMMResource.ElementBasedTextDiagnostic) diagnostic).getElement();
					...
					java.util.Collection<JTLMM.resource.JTLMM.IJTLMMQuickFix> quickFixes = textDiagnostic.getProblem().getQuickFixes();
					...
					if (!sourceIDs.isEmpty()) {
						marker.setAttribute(org.eclipse.core.resources.IMarker.SOURCE_ID, JTLMM.resource.JTLMM.util.JTLMMStringUtil.explode(sourceIDs, "|"));
					}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource) {
		for (JTLMM.resource.JTLMM.JTLMMEProblemType nextType : JTLMM.resource.JTLMM.JTLMMEProblemType.values()) {
	 ...
	 */
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, JTLMM.resource.JTLMM.JTLMMEProblemType problemType) {
		final org.eclipse.core.resources.IFile file = getFile(resource);
		if (file == null) {
			return;
		}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, final org.eclipse.emf.ecore.EObject causingObject) {
		...
		final String markerID = getMarkerID(JTLMM.resource.JTLMM.JTLMMEProblemType.UNKNOWN);
	 ...
	 * given type.
	 */
	public String getMarkerID(JTLMM.resource.JTLMM.JTLMMEProblemType problemType) {
	...
	protected void handleException(org.eclipse.core.runtime.CoreException ce) {
		if (ce.getMessage().matches("Marker.*not found.")) {
			...
			new JTLMM.resource.JTLMM.util.JTLMMRuntimeUtil().logError("Error while removing markers from resource:", ce);
		}
	}
		...
		COMMAND_QUEUE.addCommand(new JTLMM.resource.JTLMM.IJTLMMCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					JTLMM.resource.JTLMM.mopp.JTLMMPlugin.logError("Can't create marker.", e);
					return false;
				}
			}
		});
