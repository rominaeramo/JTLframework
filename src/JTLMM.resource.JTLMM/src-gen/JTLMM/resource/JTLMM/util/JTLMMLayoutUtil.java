...
public class JTLMMLayoutUtil {
	...
	public void transferLayoutInformationToModel(org.eclipse.emf.ecore.EObject element) {
		...
		for (java.util.Iterator<JTLMM.resource.JTLMM.mopp.JTLMMLayoutInformation> i = layoutInformationAdapter.getLayoutInformations().iterator(); i.hasNext(); ) {
			JTLMM.resource.JTLMM.mopp.JTLMMLayoutInformation layoutInformation = i.next();
	...
	public void transferLayoutInformationFromModel(org.eclipse.emf.ecore.EObject element) {
		...
		if (layoutReference != null) {
			...
			for (java.util.Iterator<org.eclipse.emf.ecore.EObject> i = list.iterator(); i.hasNext(); ) {
				...
				JTLMM.resource.JTLMM.mopp.JTLMMLayoutInformation layoutInformation = createLayoutInformation(layoutModelElement);
	...
	public org.eclipse.emf.ecore.EObject createLayoutInformationModelElement(JTLMM.resource.JTLMM.mopp.JTLMMLayoutInformation layoutInformation, org.eclipse.emf.ecore.EPackage layoutPackage) {
		...
		JTLMM.resource.JTLMM.grammar.JTLMMSyntaxElement syntaxElement = layoutInformation.getSyntaxElement();
		...
		layoutInformationModelElement.eSet(layoutInformationEClass.getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME), JTLMM.resource.JTLMM.grammar.JTLMMGrammarInformationProvider.getSyntaxElementID(syntaxElement));
		return layoutInformationModelElement;
	}
	
	public JTLMM.resource.JTLMM.mopp.JTLMMLayoutInformation createLayoutInformation(org.eclipse.emf.ecore.EObject layoutInformationModelElement) {
		Object object = null;
		...
		JTLMM.resource.JTLMM.grammar.JTLMMSyntaxElement syntaxElement = JTLMM.resource.JTLMM.grammar.JTLMMGrammarInformationProvider.getSyntaxElementByID((String) layoutInformationModelElement.eGet(layoutInformationModelElement.eClass().getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME)));
		if (objectFeature != null) {
			object = layoutInformationModelElement.eGet(objectFeature);
		} else if (syntaxElement instanceof JTLMM.resource.JTLMM.grammar.JTLMMPlaceholder) {
			JTLMM.resource.JTLMM.grammar.JTLMMPlaceholder placeholder = (JTLMM.resource.JTLMM.grammar.JTLMMPlaceholder) syntaxElement;
			JTLMM.resource.JTLMM.IJTLMMTokenResolver tokenResolver = new JTLMM.resource.JTLMM.mopp.JTLMMTokenResolverFactory().createTokenResolver(placeholder.getTokenName());
			JTLMM.resource.JTLMM.IJTLMMTokenResolveResult result = new JTLMM.resource.JTLMM.mopp.JTLMMTokenResolveResult();
			tokenResolver.resolve(visibleTokenText, placeholder.getFeature(), result);
			object = result.getResolvedToken();
		}
		return new JTLMM.resource.JTLMM.mopp.JTLMMLayoutInformation(syntaxElement, object, startOffset, hiddenTokenText, visibleTokenText);
	}
	
	public JTLMM.resource.JTLMM.mopp.JTLMMLayoutInformationAdapter getLayoutInformationAdapter(org.eclipse.emf.ecore.EObject element) {
