 ...
 */
public class JTLMMMinimalModelHelper {
	
	private final static JTLMM.resource.JTLMM.util.JTLMMEClassUtil eClassUtil = new JTLMM.resource.JTLMM.util.JTLMMEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + JTLMM.resource.JTLMM.util.JTLMMStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = JTLMM.resource.JTLMM.util.JTLMMListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + JTLMM.resource.JTLMM.util.JTLMMStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = JTLMM.resource.JTLMM.util.JTLMMListUtil.castListUnchecked(value);
