 ...
 */
public class JTLMMEclipseProxy {
	...
	public void getDefaultLoadOptionProviderExtensions(java.util.Map<Object, Object> optionsMap) {
		if (org.eclipse.core.runtime.Platform.isRunning()) {
			...
			org.eclipse.core.runtime.IConfigurationElement configurationElements[] = extensionRegistry.getConfigurationElementsFor(JTLMM.resource.JTLMM.mopp.JTLMMPlugin.EP_DEFAULT_LOAD_OPTIONS_ID);
			for (org.eclipse.core.runtime.IConfigurationElement element : configurationElements) {
				try {
					JTLMM.resource.JTLMM.IJTLMMOptionProvider provider = (JTLMM.resource.JTLMM.IJTLMMOptionProvider) element.createExecutableExtension("class");
					final java.util.Map<?, ?> options = provider.getOptions();
					final java.util.Collection<?> keys = options.keySet();
					for (Object key : keys) {
						JTLMM.resource.JTLMM.util.JTLMMMapUtil.putAndMergeKeys(optionsMap, key, options.get(key));
					}
				} catch (org.eclipse.core.runtime.CoreException ce) {
					new JTLMM.resource.JTLMM.util.JTLMMRuntimeUtil().logError("Exception while getting default options.", ce);
				}
			}
		}
	}
	...
	public void getResourceFactoryExtensions(java.util.Map<String, org.eclipse.emf.ecore.resource.Resource.Factory> factories) {
		if (org.eclipse.core.runtime.Platform.isRunning()) {
			org.eclipse.core.runtime.IExtensionRegistry extensionRegistry = org.eclipse.core.runtime.Platform.getExtensionRegistry();
			org.eclipse.core.runtime.IConfigurationElement configurationElements[] = extensionRegistry.getConfigurationElementsFor(JTLMM.resource.JTLMM.mopp.JTLMMPlugin.EP_ADDITIONAL_EXTENSION_PARSER_ID);
			for (org.eclipse.core.runtime.IConfigurationElement element : configurationElements) {
				try {
	 ...
	 */
	public JTLMM.resource.JTLMM.mopp.JTLMMResource getResource(org.eclipse.core.resources.IFile file) {
	...
	public void checkEMFValidationConstraints(JTLMM.resource.JTLMM.IJTLMMTextResource resource, org.eclipse.emf.ecore.EObject root) {
		...
		if (new JTLMM.resource.JTLMM.util.JTLMMRuntimeUtil().isEclipsePlatformRunning()) {
			// The EMF validation framework code throws a NPE if the validation plug-in is not
			...
			if (org.eclipse.emf.validation.internal.EMFModelValidationPlugin.getPlugin() != null) {
				try {
					...
					new JTLMM.resource.JTLMM.util.JTLMMRuntimeUtil().logError("Exception while checking contraints provided by EMF validator classes.", t);
				}
			}
		}
	}
	
	public void addStatus(org.eclipse.core.runtime.IStatus status, JTLMM.resource.JTLMM.IJTLMMTextResource resource, org.eclipse.emf.ecore.EObject root) {
		...
		if (!status.isMultiStatus() || !hasChildren) {
			if (status.getSeverity() == org.eclipse.core.runtime.IStatus.ERROR) {
				for (org.eclipse.emf.ecore.EObject cause : causes) {
					resource.addError(status.getMessage(), JTLMM.resource.JTLMM.JTLMMEProblemType.ANALYSIS_PROBLEM, cause);
				}
			}
			if (status.getSeverity() == org.eclipse.core.runtime.IStatus.WARNING) {
				for (org.eclipse.emf.ecore.EObject cause : causes) {
					resource.addWarning(status.getMessage(), JTLMM.resource.JTLMM.JTLMMEProblemType.ANALYSIS_PROBLEM, cause);
				}
			}
		}
		...
		if (uri != null && uri.isPlatform()) {
			...
			if (platformResource instanceof org.eclipse.core.resources.IFile) {
				...
				try {
					...
					new JTLMM.resource.JTLMM.util.JTLMMRuntimeUtil().logWarning("Could not determine encoding of platform resource: " + uri.toString(), ce);
				}
			}
		}
		return null;
