...
public class JTLMMDefaultResolverDelegate<ContainerType extends org.eclipse.emf.ecore.EObject, ReferenceType extends org.eclipse.emf.ecore.EObject> {
	...
	private static JTLMM.resource.JTLMM.mopp.JTLMMMetaInformation metaInformation = new JTLMM.resource.JTLMM.mopp.JTLMMMetaInformation();
	
	private JTLMM.resource.JTLMM.IJTLMMNameProvider nameProvider = metaInformation.createNameProvider();
	 ...
	 */
	protected void resolve(String identifier, ContainerType container, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, JTLMM.resource.JTLMM.IJTLMMReferenceResolveResult<ReferenceType> result) {
		try {
	 ...
	 */
	protected boolean tryToResolveIdentifierInObjectTree(String identifier, org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EObject root, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, JTLMM.resource.JTLMM.IJTLMMReferenceResolveResult<ReferenceType> result, boolean checkRootFirst) {
	...
	protected boolean tryToResolveIdentifierAsURI(String identifier, ContainerType container, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, JTLMM.resource.JTLMM.IJTLMMReferenceResolveResult<ReferenceType> result) {
		org.eclipse.emf.ecore.EClass type = reference.getEReferenceType();
	...
	protected boolean tryToResolveIdentifierInGenModelRegistry(String identifier, ContainerType container, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, JTLMM.resource.JTLMM.IJTLMMReferenceResolveResult<ReferenceType> result) {
		org.eclipse.emf.ecore.EClass type = reference.getEReferenceType();
	...
	protected boolean checkElement(org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EObject element, org.eclipse.emf.ecore.EReference reference, int position, org.eclipse.emf.ecore.EClass type, String identifier, boolean resolveFuzzy, boolean checkStringWise, JTLMM.resource.JTLMM.IJTLMMReferenceResolveResult<ReferenceType> result) {
		...
		if (exactMatch == null) {
			...
			if (similarMatch != null) {
				...
				result.addQuickFix(new JTLMM.resource.JTLMM.mopp.JTLMMChangeReferenceQuickFix("Replace with " + similarMatch, "IMG_TOOL_FORWARD", container, reference, oldTarget, element));
			}
			return true;
		}
	...
	protected String produceDeResolveErrorMessage(org.eclipse.emf.ecore.EObject refObject, org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EReference reference, JTLMM.resource.JTLMM.IJTLMMTextResource resource) {
	...
	protected JTLMM.resource.JTLMM.IJTLMMReferenceCache getCache(org.eclipse.emf.ecore.EObject object) {
		org.eclipse.emf.ecore.EObject root = org.eclipse.emf.ecore.util.EcoreUtil.getRootContainer(object);
		org.eclipse.emf.common.notify.Adapter adapter = JTLMM.resource.JTLMM.util.JTLMMEObjectUtil.getEAdapter(root, JTLMM.resource.JTLMM.analysis.JTLMMReferenceCache.class);
		JTLMM.resource.JTLMM.analysis.JTLMMReferenceCache cache = JTLMM.resource.JTLMM.util.JTLMMCastUtil.cast(adapter);
		if (cache != null) {
			return cache;
		} else {
			// cache does not exist. create a new one.
			cache = new JTLMM.resource.JTLMM.analysis.JTLMMReferenceCache(nameProvider);
	...
	protected boolean isSimilar(String identifier, Object attributeValue) {
		if (attributeValue != null && attributeValue instanceof String) {
			String name = (String) attributeValue;
			if (JTLMM.resource.JTLMM.util.JTLMMStringUtil.computeLevenshteinDistance(identifier, name) <= MAX_DISTANCE) {
				return true;
			}
		}
		return false;
